...
func ParseOutput(ctx context.Context, caffOutput io.Reader) (*image.RGBA, error) {
    ...
    if height < 1 || width < 1 { return nil, ErrZeroDimension }
    ...
    if len(pixBytes) < expectedBytes { return nil, ErrTruncatedPixels }
    if len(pixBytes) > expectedBytes { return nil, ErrTooManyBytes }
